...
def capture_from_camera_and_show_images():
    ...
    while not stop:
        ...
        _, binary_img = cv2.threshold(dif_img, threshold, 1.0, cv2.THRESH_BINARY)
        #binary_img = dif_img > threshold
        foreground_pixels = np.sum(binary_img)
        total_pixels = binary_img.size
        F = foreground_pixels / total_pixels

        if F > alert_value:
            cv2.putText(new_frame, "Change Detected!", (50, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
        ...
        show_in_moved_window('Difference image', img_as_ubyte(binary_img), 300, 10)
        #show_in_moved_window('Difference image', dif_img, 200, 10)
